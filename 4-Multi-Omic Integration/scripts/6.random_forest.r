...
MetaG.MetaB.links <- fread("4_MetaG.MetaB.modules.linked.txt",select = 1, col.names = "V1") %>% unique() %>%
  mutate(MetaG.module = sapply(strsplit(V1, "_", fixed = T), "[[", 1),
         MetaB.module = sapply(strsplit(V1, "_", fixed = T), "[[", 2))


MetaB.HostT.links <- fread("4_MetaB.HostT.modules.linked.txt",select = 1, col.names = "V1")  %>% 
...
meta <- fread("meta.mediation.NUE.txt") %>% select(SampleID, NUE) 
...
for(i in c(1:nrow(link.df))){ # nrow(link.df)
  ...
  performance <- performance  %>% relocate(MetaG.MetaB.HostT.link, mtry, trees)
