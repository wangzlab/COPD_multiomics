...
for(i in c(1:length(KO2EC_list))){
  ...
  for(ec in ecs){
    products <- unique(append(products, EC.Products_list[[ec]]) )
    substrates <- unique(append(substrates, EC.Substrates_list[[ec]]) )
    subs.prod <- unique(append(subs.prod, EC.SubsProd_list[[ec]]) )
