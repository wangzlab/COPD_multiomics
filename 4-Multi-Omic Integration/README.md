...
metagenome.gct
KEGG_modules.gmt
metadata.txt (containing a column named SampleID and variables of confounders and a column indicating disease state)

Script: 
Rscript 2.significant.metaG.modules.r

Output: 
2_significant_metaG_modules.RData (a R data file containing the list of metagenomic modules associated with COPD)
```
...
Output: 
2_significant_metaB_modules.RData (a R data file containing the list of metabolomic modules associated with COPD)
```

```
Input: 
1_hostT.module_eigengene.txt (module-level metabolome profile output from step 1)

Script: 
Rscript 2.significant.hostT.modules.r

Output: 
2_significant_hostT_modules.RData (a R data file containing the list of metabolomic modules associated with COPD)
```
...
Output: 
2_significant_hostP_modules.RData (a R data file containing the list of protein features associated with COPD)
```
...
meta.mediation.NEU.txt (containing metadata for the endpoint for the mediation analysis [NEU] and demographic confounders)
...
KO2CMPD.lists.RData (R data file linking KO IDs and compounds generated from previous step)
...
metabolome.txt
metagenome.gct
Metabo.KEGGModule.match.txt (manually curated file storing KEGG compound ID-module mapping information)

Script: 
Rscript 4.MetaG.MetaB.link.r

Output: 
4_MetaG.MetaB.modules.linked.txt (linked MetaG-MetaB modules and the MetaG and MetaB features that link the two modules)
```
...
Output: 
4_MetaB.HostT.modules.linked.txt (linked MetaB-HostT modules and the MetaB and HostT features that link the two modules)
```
...
Output: 
4_HostT.module_HostP.protein.linked.txt (linked MetaB-HostT modules and the gene/protein ID that links the two modules)
```
...
```
Input: 
3_MetaG.MetaB.modules.linked.txt
1_metaG-combined.gct
1_metaB.module_eigengene.txt
LOSO_metaG_DR (a directory containing all dimensionality reduced MetaG profiles named as speciesX-combined.gct, one for each species excluded)

Script: 
Rscript 5.LOSO.delta.r

Output: 
5_LOSO_delta.spearman.r.txt (contains the delta spearman's r and the z-score for the MetaG-MetaB pair when each species was iteratively removed)
```
