...
excel_sheets("Fig S4 data.xlsx")

# Fig S4 ------------------------------------------
# Fig S4. LOSO density plot #######
rm(list = ls())
dat.hist <- read_excel("Fig S4 data.xlsx", sheet = "LOSO")
colnames(dat.hist) <- c("Figs_map", "Figs_color","species","X1")
dat.hist <- dat.hist %>% mutate(X2=X1, Y1=-0.002, Y2=-0.009)

taxonomy_df <- read_excel("Fig S4 data.xlsx", sheet = "taxonomy")

genus <- sapply(strsplit(dat.hist$species," ", fixed = T),"[[", 1)
...
library(ggpubr)
FigS4.LOSO <- ggarrange(histP_MetaG_P00380, histP_MetaG_M00044, histP_MetaG_P00250, histP_MetaG_P00564, ncol = 1)
#ggsave(FigS4.LOSO,device = "pdf", filename = "FigS4.KOcontri.pdf", width = 3, height = 10)


# Fig S4. KO contribution #######

zscore_df <- read_excel("Fig S4 data.xlsx", sheet = "KO contrib" )
...
FigS4.KOcontri <- ggplot(zscore.top3_df) +
