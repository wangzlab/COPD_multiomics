...
excel_sheets("Fig S6 Source Data.xlsx")

# Fig S6 ------------------------------------------
# Fig S6. LOSO density plot #######
rm(list = ls())
dat.hist <- read_excel("Fig S6 Source Data.xlsx", sheet = "LOSO")
colnames(dat.hist) <- c("Figs_map", "Figs_color","species","X1")
dat.hist <- dat.hist %>% mutate(X2=X1, Y1=-0.002, Y2=-0.009)

taxonomy_df <- read_excel("Fig S6 Source Data.xlsx", sheet = "taxonomy")
...
library(ggpubr)
FigS6.LOSO <- ggarrange(histP_MetaG_P00380, histP_MetaG_M00044, histP_MetaG_P00250, histP_MetaG_P00564, ncol = 1)


# Fig S6. KO contribution #######

zscore_df <- read_excel("Fig S6 Source Data.xlsx", sheet = "KO contrib" )
...
FigS6.KOcontri <- ggplot(zscore.top3_df) +
